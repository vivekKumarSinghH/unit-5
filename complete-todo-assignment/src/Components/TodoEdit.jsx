...
import {  EditTodo } from "../Redux/Todos/action";
...
export default function TodoEdit() {
  ...
  useEffect(() => {
    axios
      .get(`http://localhost:3002/todos/${id}`)
      .then((res) =>
        dispatch({ type: "UPDATE_INIT_STATE", payload: res.data })
      );
  }, []);

  return (
    <div className="flex gap-2 w-full  mt-5 justify-around">
      <Sidebar todos={todos} />
      <div className="todo flex">
        <div>
          <input
            className="border-black border-2 rounded w-[70%] px-4 mt-4"
            placeholder="Title"
            type="text"
            onChange={(e) => {
        ...
        <div>
          <p className="mt-4 text-2xl">EDIT SUBTASKS</p>
          <div className="flex h-[80px] my-5 gap-2">
            <input
              className="border-black border-2 rounded w-[60%] h-full px-4 "
        ...
        <div className="m-4">
          <input
            className="border-black border-2 rounded w-full  my-4  p-4  block"
            placeholder="Date"
            type="date"
            onChange={(e) => {
          ...
          <button
            onClick={() => {
              reduxDispatch(EditTodo({ id,payload:state }));
              dispatch({ type: "RESET" });
